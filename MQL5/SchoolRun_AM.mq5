...
// Morning session: draws lines at the high/low of a chosen M15 candle (server time).
input int  DaysToDraw            = 10;        // Days back (incl. today)
input int  TargetHourServer      = 10;        // e.g., 10 -> candle 10:15–10:30
input int  TargetMinuteServer    = 15;        // 15 -> selects :15 M15 bar
                ...
                const int &spread[])
{
   ...
   for(int d=0; d<DaysToDraw; d++)
   {
      ...
      if(DebugLog)
         PrintFormat("AM: D1=%s target=%s hi=%.5f lo=%.5f",
